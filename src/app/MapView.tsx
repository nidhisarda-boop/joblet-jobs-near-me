...
export default function MapView({
  ...
  onSel: (j: Job) => void; radius: number;
}) {
  const mj = jobs.filter((j) => j.lat && j.lng);
  const cLat = mj.length ? mj.reduce((s, j) => s + (j.lat || 0), 0) / mj.length : 39.8;
  const cLng = mj.length ? mj.reduce((s, j) => s + (j.lng || 0), 0) / mj.length : -98.6;
